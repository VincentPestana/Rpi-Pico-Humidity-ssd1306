...
def build_html_page():
    ...
    return (
        ...
        "// In-flight request management (10s timeout + cancellation)\n"
        "let reqController=null, reqStarted=0;\nconst REQ_TIMEOUT=10000;\n"
        ...
        "async function tick(){\n"
        "  try{\n"
        "    // If a request is still in flight, either wait or abort after 10s\n"
        "    if(reqController){\n"
        "      if(Date.now()-reqStarted>REQ_TIMEOUT){try{reqController.abort()}catch(e){} reqController=null;}\n"
        "      else{ return; }\n"
        "    }\n"
        "    // Start a new request with AbortController if available\n"
        "    if('AbortController' in window){reqController=new AbortController()} else {reqController=null}\n"
        "    reqStarted=Date.now();\n"
        "    const opts={cache:'no-store'}; if(reqController){opts.signal=reqController.signal}\n"
        "    const r=await fetch('/data?points='+fetchPts, opts);\n"
        "    const d=await r.json();\n"
        "    draw(d);\n"
        "  }catch(e){/* ignore */}\n"
        "  finally{ reqController=null }\n"
        "}\n"
